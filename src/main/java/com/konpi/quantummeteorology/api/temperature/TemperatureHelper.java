...
public class TemperatureHelper {
	 ...
	 *
	 * @param world
	 *            The world to check the temperature of
	 * @param pos
	 *            The position to check the temperature at
	 * @param monitor
	 *            The modifier monitor (may be null)
	 * @return The temperature at the provided position
	 */
	public static Temperature getTargetAtPos(@Nonnull World world, @Nonnull BlockPos pos,
			@Nullable IModifierMonitor monitor) {
		return new Temperature(MathHelper.clamp(getTargetAtPosUnclamped(world, pos, monitor).getRawValue(), 0,
	 ...
	 * @return The temperature at the provided position
	 */
	public static Temperature getTargetAtPosUnclamped(@Nonnull World world, @Nonnull BlockPos pos,
			@Nullable IModifierMonitor monitor) {
		if (monitor == null)
			monitor = DUMMY_MONITOR;

		int targetTemperature = TemperatureScale.getScaleMidpoint();
		monitor.addEntry(new IModifierMonitor.Context("equilibrium", "Equilibrium", new Temperature(0),
				new Temperature(TemperatureScale.getScaleMidpoint())));

		for (ITemperatureModifier modifier : TemperatureHelper.getTemperatureModifiers().values()) {
			if (!modifier.isPlayerSpecific())
				targetTemperature = modifier
						.applyEnvironmentModifiers(world, pos, new Temperature(targetTemperature), monitor)
						.getRawValue();
		}
	...
	public static List<ITemperatureRegulator> getTemperatureRegulators(World world) {
		...
		for (TileEntity tileEntity : world.tickableTileEntities) {
			if (tileEntity instanceof ITemperatureRegulator)
	...
	public static boolean isPosClimatisedForTemp(World world, BlockPos pos, Temperature temperature) {
		for (ITemperatureRegulator regulator : getTemperatureRegulators(world)) {
			if (regulator.getRegulatedTemperature().getRawValue() >= temperature.getRawValue()
					&& regulator.isPosRegulated(pos)) {
				return true;
			}
		}
