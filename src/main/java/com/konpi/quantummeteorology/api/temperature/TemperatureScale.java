...
public class TemperatureScale {
	...
	/**
	 * Returns an index within a range, given a position in the overall temperature
	 * scale. When reversed, 1.0 values are when the input position is closer to 0
	 * in the overall temperature scale
	 */
	public static int getRangeIndex(int scalePos, boolean reverseEnd) {
		...
		return Math.abs((reverseEnd ? (temperatureRange.getRangeSize() - 1) : 0)
				- (scalePos - rangeStarts[temperatureRange.ordinal()]));
	}

	/**
	 * Returns on a scale of 0.0F to 1.0F the location of a temperature within the
	 * current range When reversed, 1.0 values are when the input position is closer
	 * to 0 in the overall temperature scale
	 */
	...
	public static boolean isScalePosInRange(int scalePos, TemperatureRange startRange, TemperatureRange endRange) {
		return scalePos >= rangeStarts[startRange.ordinal()]
				&& scalePos <= (rangeStarts[endRange.ordinal()] + endRange.rangeSize - 1);
	}
	...
	public static int getRateForTemperatures(int currentScalePos, int targetScalePos) {
		// the greater the difference between the current temperature and the target
		// temperature,
	...
	public static int getScaleMidpoint() {
		return TemperatureScale.getScaleTotal() / 2;
	}

	/**
	 * @return The maximum number of ticks before the temperature changes
	 */
	public static int getBaseTemperatureChangeTicks() {
		// return
		// SyncedConfig.getIntValue(TemperatureOption.BASE_TEMPERATURE_CHANGE_TICKS);
		return 500;
	}

	/**
	 * @return The maximum number of ticks to reduce the base rate by
	 */
