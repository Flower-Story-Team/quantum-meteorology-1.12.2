package com.konpi.quantummeteorology.common.handler.temperature;
...
import com.konpi.quantummeteorology.common.temperature.TemperatureDebugger;
...
public class TemperatureStatTableHandler {
	...
	public void onPostRenderOverlay(RenderGameOverlayEvent.Post event) {
		...
		TemperatureHandler temperatureHandler = (TemperatureHandler) player.getCapability(Capabilities.TEMPERATURE,
				null);
		...
		if (event.getType() == ElementType.ALL && debugger.isGuiVisible()) {
			String targetProgress = "Target " + TextFormatting.RED + currentTemp + "/" + debugger.targetTemperature
					+ getCappedText(debugger.targetTemperature);
			...
			for (Entry<String, IModifierMonitor.Context> entry : debugger.modifiers.entrySet()) {
				TEMPERATURE_TABLE.addRow(entry.getValue().description, entry.getValue().endTemperature.getRawValue()
						- entry.getValue().startTemperature.getRawValue());
			}

			int gradient = TemperatureScale.getRateForTemperatures(currentTemp, debugger.targetTemperature)
					- TemperatureScale.getAdjustedBaseRate(currentTemp);
			if (gradient != 0)
				RATE_TABLE.addRow("Gradient", gradient);

			int extremityModifier = TemperatureScale.getAdjustedBaseRate(currentTemp)
					- TemperatureScale.getBaseTemperatureChangeTicks();
			if (extremityModifier != 0)
		...
		public void drawTable(int x, int y) {
			...
			for (int i = 0; i < this.rows.size() + 1; i++) {
				...
				if (i == 0) {
					...
					fontRenderer.drawString(formattedValue, textEnd - fontRenderer.getStringWidth(formattedValue),
							rowTopY, 553648127);
				}
			}
		}
