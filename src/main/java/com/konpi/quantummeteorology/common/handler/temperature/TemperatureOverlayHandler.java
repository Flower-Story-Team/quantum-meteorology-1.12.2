...
import com.konpi.quantummeteorology.common.util.RenderUtils;
...
public class TemperatureOverlayHandler {
	...
	public static final ResourceLocation ICE_VIGNETTE = new ResourceLocation("flower:textures/gui/ice_vignette.png");
	public static final ResourceLocation FIRE_VIGNETTE = new ResourceLocation("flower:textures/gui/fire_vignette.png");
	...
	private void drawTemperature(int width, int height, Temperature temperature) {
		...
		if (flashCounter > (long) updateCounter) {
			if (updateDelta > 6 && updateDelta / 3L % 2L == 1L) {
				RenderUtils.drawTexturedModalRect(left, top,
						16 * (temperatureIcon.backgroundIndex + flashType.backgroundShift), 0, 16, 16);
				RenderUtils.drawTexturedModalRect(left, top,
						16 * (temperatureIcon.foregroundIndex + flashType.foregroundShift), 0, 16, 16);
			...
			RenderUtils.drawTexturedModalRect(0, 0, 16 * (16 - updateDelta), 16 * 15, 16, 16);
	...
	private void renderColouredBall(int x, int y, Temperature temperature, int textureShift) {
		...
		if (temperatureRange != TemperatureRange.MILD) {
			...
			RenderUtils.drawTexturedModalRect(x, y,
					16 * ((temperatureRange == TemperatureRange.COOL ? 8 : 9) + textureShift), 16 * 1, 16, 16);
