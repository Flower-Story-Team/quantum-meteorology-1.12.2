...
public class WeatherModifier extends TemperatureModifier {
	...
	public Temperature applyPlayerModifiers(EntityPlayer player, Temperature initialTemperature,
		...
		if (player.isWet()) {
			...
			// monitor.addEntry(new IModifierMonitor.Context(this.getId(), "Wet",
			// initialTemperature, new Temperature(newTemperatureLevel)));
			// }
			...
			monitor.addEntry(new IModifierMonitor.Context(this.getId(), "Snow", initialTemperature,
