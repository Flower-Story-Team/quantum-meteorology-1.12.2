package com.konpi.quantummeteorology.common.temperature;
...
import com.konpi.quantummeteorology.api.temperature.ITemperatureModifier;
import com.konpi.quantummeteorology.api.temperature.Temperature;
import com.konpi.quantummeteorology.api.temperature.TemperatureHelper;
import com.konpi.quantummeteorology.api.temperature.TemperatureScale;
import com.konpi.quantummeteorology.api.temperature.TemperatureScale.TemperatureRange;
import com.konpi.quantummeteorology.common.network.UpdateStateMessage;
import com.konpi.quantummeteorology.common.temperature.modifier.TemperatureModifier;
import com.konpi.quantummeteorology.common.temperature.modifier.TemperatureModifier.ExternalModifier;
...
import static com.konpi.quantummeteorology.api.temperature.TemperatureHelper.registerTemperatureModifier;

public class TemperatureHandler extends StateHandlerBase implements ITemperature {
	...
	public void update(EntityPlayer player, World world, Phase phase) {
		if (phase == Phase.END && !world.isRemote) {
			...
			if (incrementTemperature) {
				if (!player.isCreative() && (world.getDifficulty() != EnumDifficulty.PEACEFUL)) {
	...
	public int getPlayerTarget(EntityPlayer player) {
		int targetTemperature = TemperatureHelper.getTargetAtPosUnclamped(player.world, player.getPosition(), debugger)
				.getRawValue();

		for (ITemperatureModifier modifier : TemperatureHelper.getTemperatureModifiers().values()) {
			if (modifier.isPlayerSpecific())
				targetTemperature = modifier.applyPlayerModifiers(player, new Temperature(targetTemperature), debugger)
						.getRawValue();
		}
		...
		debugger.addEntry(new IModifierMonitor.Context("climatisation", "Climatisation", preClimatisationTemp,
				new Temperature(targetTemperature)));
	...
	private void addPotionEffects(EntityPlayer player) {
		...
		// && (temperatureLevel < prevTemperatureLevel ||
		// !player.isPotionActive(TANPotions.hypothermia))) {
		...
		// && (temperatureLevel > prevTemperatureLevel ||
		// !player.isPotionActive(TANPotions.hyperthermia))) {
	...
	public IMessage createUpdateMessage() {
		NBTTagCompound data = (NBTTagCompound) Capabilities.TEMPERATURE.getStorage().writeNBT(Capabilities.TEMPERATURE,
				this, null);
		return new UpdateStateMessage(Capabilities.TEMPERATURE, data);
	}
