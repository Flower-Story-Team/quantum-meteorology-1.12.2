...
public class BiomeModifier extends TemperatureModifier {
	...
	public Temperature applyEnvironmentModifiers(World world, BlockPos pos, Temperature initialTemperature,
		...
		// int temperatureModifier = (int)Math.round((biomeTemp * 2.0F - 1.0F) *
		// ModConfig.temperature.maxBiomeTempOffset);

		// Apply underground coefficient
		// if (world.provider.isSurfaceWorld() &&
		// ModConfig.temperature.enableUndergroundEffect)
		// temperatureModifier =
		// Math.round(TerrainUtils.getAverageUndergroundCoefficient(world, pos) *
		// temperatureModifier);
		// newTemperatureLevel += temperatureModifier;

		monitor.addEntry(new IModifierMonitor.Context(this.getId(), "Biome Temperature", initialTemperature,
