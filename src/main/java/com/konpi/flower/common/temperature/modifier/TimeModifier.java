...
public class TimeModifier extends TemperatureModifier {
	...
	public Temperature applyEnvironmentModifiers(World world, BlockPos pos, Temperature initialTemperature,
		...
		// Reaches the highest point during the middle of the day and at midnight.
		...
		// if (world.provider.isSurfaceWorld() && ((timeNorm < 0 &&
		// ModConfig.temperature.enableNightTimeModifier)
		// || (timeNorm > 0 && ModConfig.temperature.enableDayTimeModifier))) {
		// int temperatureModifier = (int) (ModConfig.temperature.timeModifier *
		// timeNorm
		// * (Math.max(1.0F, extremityModifier *
		// ModConfig.temperature.timeExtremityMultiplier)));

		// Apply underground coefficient
		// if (ModConfig.temperature.enableUndergroundEffect)
		// temperatureModifier = Math
		// .round(TerrainUtils.getAverageUndergroundCoefficient(world, pos) *
		// temperatureModifier);
		// newTemperatureLevel += temperatureModifier;
		// }

		monitor.addEntry(new IModifierMonitor.Context(this.getId(), "Time", initialTemperature,
